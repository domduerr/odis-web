...
use crate::core::layout_math::{
    dimdraw_drawing, scale_coordinates_to_viewport, sugiyama_drawing,
    Dimensions as LayoutDimensions,
};
...
fn nodes_from_positions(
    ...
    node_positions: &[(usize, f64, f64)],
    dimensions: &Dimensions,
) -> Vec<Node> {
    let mut coords_by_id: Vec<Option<(f64, f64)>> = vec![None; graph_nodes.len()];
    for &(id, x, y) in node_positions {
        if id < coords_by_id.len() {
            coords_by_id[id] = Some((x, y));
        }
    }
    ...
    graph_nodes
        .iter()
        .map(|node| {
            if let Some((x, y)) = coords_by_id.get(node.id).and_then(|v| *v) {
...
fn build_reduced_graph_nodes(
    ...
    // Reduced object labeling: place each object at the concept generated by {g}''.
    for obj_idx in 0..context.objects.len() {
        ...
        {
            object_labels[node_idx].push(context.objects[obj_idx].clone());
        }
    }

    // Reduced attribute labeling: place each attribute at the concept generated by {m}'.
    for attr_idx in 0..context.attributes.len() {
        ...
        {
            attribute_labels[node_idx].push(context.attributes[attr_idx].clone());
        }
    }
    ...
    (0..node_count)
        .map(|id| GraphNode {
            id,
            label: (
                if object_labels[id].is_empty() {
                    None
                } else {
                    Some(object_labels[id].join(", "))
                },
                if attribute_labels[id].is_empty() {
                    None
                } else {
                    Some(attribute_labels[id].join(", "))
                },
            ),
        })
        .collect()
...
pub fn GraphComp(
    ...
    let (lattice, edges, graph_nodes) = if let Some(lattice) = lattice_option {
        let graph_nodes = build_reduced_graph_nodes(&lattice, &context);
    ...
    {
        compute_raw_layout_coordinates(lattice_ref, layout_algorithm.get_untracked())
    } else {
        Vec::new()
    });
    ...
    Effect::new(move || {
        ...
        let raw_coords = if let Some(lattice_ref) = lattice_for_layout_writer.as_ref() {
            compute_raw_layout_coordinates(lattice_ref, algo)
        } else {
            Vec::new()
        };
    ...
    Effect::new(move || {
        ...
        let node_positions_scaled = scale_coordinates_to_viewport(&raw_coords, layout_dims);
